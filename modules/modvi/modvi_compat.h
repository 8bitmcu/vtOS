/*
 * modvi_compat.h -- force-included (via -include, see micropython.cmake)
 ...
 * translation units (never modvi.c/modvi_term.c/modvi_vfs.c,
 ...
 * macros (`#define realloc(p, n) modvi_realloc((p), (n))`) break the
 ...
 * *modvi_realloc((ptr), (size));`. A bare identifier substitution
 * instead turns that same declaration into `void *modvi_realloc(void
...
#ifndef MODVI_COMPAT_H
#define MODVI_COMPAT_H

#include <stddef.h>

void *modvi_malloc(size_t n);
void *modvi_realloc(void *p, size_t n);
void modvi_free(void *p);

#define malloc modvi_malloc
#define realloc modvi_realloc
#define free modvi_free
