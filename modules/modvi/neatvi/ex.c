...
int xts = 8;			/* tabstop */
static EXT_RAM_BSS_ATTR char xkwd[EXLEN]; /* the last searched keyword */
static EXT_RAM_BSS_ATTR char xrep[EXLEN]; /* the last replacement */
...
static char *lbuf_write(struct lbuf *lb, int beg, int end, char *path, int force, long ts)
{
	...
	} else if ((fd = vfs_open(path, "wb")) < 0) {
		// "wb" (not "ab"/"r+b") truncates on open, same as the original
		// open(path, O_WRONLY|O_CREAT) + lbuf_wr()'s own trailing
		// ftruncate() -- see lbuf_wr()'s comment.
		return "write failed: cannot create file";
	} else if (lbuf_wr(lb, fd, beg, end) != 0 || vfs_close(fd) != 0) {
		vfs_close(fd);
		return "write failed";
	}
	return NULL;
...
static int ec_edit(char *loc, char *cmd, char *arg, char *txt)
{
	...
	fd = vfs_open(ex_path(), "rb");
	if (fd >= 0) {
		int rd = lbuf_rd(xb, fd, 0, lbuf_len(xb));
		vfs_close(fd);
		// The read-confirmation message ("R%04d <path>") is dropped here:
		// redundant now that the status bar itself always shows the
		// filename and position. The failure case is kept -- that's a
...
static EXT_RAM_BSS_ATTR char tag_path[TAGCNT][TAGLEN];
static EXT_RAM_BSS_ATTR char tag_name[TAGCNT][TAGLEN];
...
static int ec_source(char *loc, char *cmd, char *arg, char *txt)
{
	...
	char buf[1 << 10];
	struct sbuf *sb;
	int fd = path[0] ? vfs_open(path, "rb") : -1;
	long nr;
	if (fd < 0)
		return 1;
	sb = sbuf_make();
	while ((nr = vfs_read(fd, buf, sizeof(buf))) > 0)
		sbuf_mem(sb, buf, nr);
	vfs_close(fd); // upstream never closed this fd -- fixed while porting
...
{
	// next_pos is a file-scope static (same class of bug as w_path/xquit
	// above): a blank session (no filename) leaves ex_next("e", 0)
	// computing idx=-1 and storing it here. Left unreset, the *next*
	// session's ex_next() reads old=next_pos=-1 and indexes next[-1] --
	// an out-of-bounds read instead of next[0] -- ignoring that session's
	// real filename and opening blank/unnamed instead.
	next_pos = 0;
	next = files;
	if (ex_next("e", 0))
		return 1;
