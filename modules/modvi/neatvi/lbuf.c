...
{
	long nw = 0, nc = 0;
	while (nw < sz && (nc = vfs_write(fd, buf + nw, sz - nw)) >= 0)
		nw += nc;
	return nc >= 0 ? nw : -1;
}
...
int lbuf_wr(struct lbuf *lbuf, int fd, int beg, int end)
{
	...
	if (buf_len > 0 && write_fully(fd, buf, buf_len) < 0)
		return 1;
	// No ftruncate() here (and no VFS equivalent needed): every caller
	// opens fd via vfs_open(path, "w"), which -- like Python's own
	// open(path, "w") -- already truncates on open, so a full rewrite
	// from position 0 always ends up exactly `sz` bytes long. See
	// ex.c's lbuf_write().
	return 0;
}
