...
unsigned int defaultfg = 7; /* Usually white/light gray */
unsigned int defaultbg = 0; /* Usually black */
...
void selextend(int col, int row, int type, int done) {
  ...
  if (oldey != sel.oe.y || oldex != sel.oe.x || oldtype != sel.type ||
      sel.mode == SEL_EMPTY)
    tsetdirt(SMIN(sel.nb.y, oldsby), SMAX(sel.ne.y, oldsey));
...
void selnormalize(void) {
  ...
  if (sel.type == SEL_REGULAR && sel.ob.y != sel.oe.y) {
    ...
    sel.nb.x = SMIN(sel.ob.x, sel.oe.x);
    sel.ne.x = SMAX(sel.ob.x, sel.oe.x);
  }
  sel.nb.y = SMIN(sel.ob.y, sel.oe.y);
  sel.ne.y = SMAX(sel.ob.y, sel.oe.y);
...
void kscrollup(const Arg *a) {
  int n = a->i;
  static int history_is_full = 0;

  if (n < 0)
    n = term.row + n;

  if (term.histi == HISTSIZE - 1)
    history_is_full = 1;

  int max_scroll = history_is_full ? HISTSIZE : term.histi;

  if (term.scr < max_scroll) {
    if (term.scr + n > max_scroll) {
      n = max_scroll - term.scr;
    }
...
void tdumpline(int n) {
  ...
  bp = &term.line[n][0];
  end = &bp[SMIN(tlinelen(n), term.col) - 1];
  if (bp != end || bp->u != ' ') {
    for (; bp <= end; ++bp)
...
void tputc(Rune u) {
  ...
  if (u < 127 || !IS_SET(MODE_UTF8)) {
    ...
    len = utf8encode(u, c);
    if (!control && (width = st_wcwidth(u)) == -1)
      width = 1;
  }
...
void tresize(int col, int row) {
  int i, j;
  int minrow = SMIN(row, term.row);
  int mincol = SMIN(col, term.col);
  int *bp;
  TCursor c;
