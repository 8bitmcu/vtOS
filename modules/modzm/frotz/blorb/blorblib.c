...
bb_err_t bb_create_map(FILE *file, bb_map_t **newmap)
{
    ...
    *newmap = NULL;
    ...
    err = modzm_fseek(file, 0, 0);
    if (err)
        return bb_err_Read;

    readlen = modzm_fread(buffer, sizeof(uint32), 3, file);
    if (readlen != 3)
        return bb_err_Read;
    ...
    while (nextpos < totallength) {
        ...
        err = modzm_fseek(file, nextpos, 0);
        if (err)
            return bb_err_Read;

        readlen = modzm_fread(buffer, sizeof(uint32), 2, file);
        if (readlen != 2)
            return bb_err_Read;
    ...
    switch (method) {
        ...
        case bb_method_Memory:
            if (!chu->ptr) {
                ...
                err = modzm_fseek(map->file, chu->datpos, 0);
                if (err)
                    return bb_err_Read;

                readlen = modzm_fread(dat, 1, chu->len, map->file);
                if (readlen != chu->len)
                    return bb_err_Read;
