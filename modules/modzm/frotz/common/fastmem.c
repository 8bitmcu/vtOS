...
char auxiliary_name[MAX_FILE_NAME + 1] = DEFAULT_AUXILIARY_NAME;
...
void init_setup(void)
{
	...
	f_setup.script_name_override = NULL;
...
void init_memory(void)
{
	...
	for (i = 0; i < 64 ; i++) {
		if (modzm_fread(zmp + i, 1, 1, story_fp) != 1) {
			os_fatal ("Story file read error");
		}
		zmp[i] &= 0xff; /* No nine-bit craziness here! */
	}
#else
	if (modzm_fread(zmp, 1, 64, story_fp) != 64)
		os_fatal("Story file read error");
#endif
	...
	for (size = 64; size < story_size; size++) {
		if (modzm_fread(zmp + size, 1, 1, story_fp) != 1) {
			os_fatal("Story file read error");
		}
	...
	for (size = 64; size < story_size; size += n) {
		...
		SET_PC(size);
		if (modzm_fread(pcp, 1, n, story_fp) != n)
			os_fatal("Story file read error");
    modzm_yield(1);
	}
#endif
	...
	modzm_fseek(story_fp, 64, SEEK_SET);
...
void z_restart(void)
{
	...
	if (!first_restart) {
		...
		if (modzm_fread(zmp, 1, z_header.dynamic_size, story_fp) != z_header.dynamic_size)
			os_fatal ("Story file read error");
...
void z_restore(void)
{
	...
	if (zargc != 0) {
		...
		/* Open auxiliary file */
		if ((gfp = modzm_fopen (new_name, "rb")) == NULL)
			goto finished;

		/* Load auxiliary file */
		success = modzm_fread (zmp + zargs[0], 1, zargs[1], gfp);
...
void z_save(void)
{
	...
	if (zargc != 0) {
		...
		/* Open auxiliary file */
		if ((gfp = modzm_fopen(new_name, "wb")) == NULL)
			goto finished;
	 ...
	 */
		...
		if (modzm_fclose(gfp) == EOF || ferror(story_fp)) {
