 ...
 *
 */
void script_open(bool noprompt)
{
	...
	if ((z_header.version >= V5 || !script_valid)  && !noprompt)  {
	...
	if (f_setup.script_name_override != NULL)
		f_setup.script_name = strdup(f_setup.script_name_override);

	/* Opening in "at" mode doesn't work for script_erase_input... */
	if ((sfp = modzm_fopen (f_setup.script_name, "r+t")) != NULL ||
	    (sfp = modzm_fopen(f_setup.script_name, "w+t")) != NULL) {
		modzm_fseek (sfp, 0, SEEK_END);
		z_header.flags |= SCRIPTING_FLAG;
		script_valid = TRUE;
		ostream_script = TRUE;
		script_width = 0;
...
void script_erase_input(const zchar *buf)
{
	...
	modzm_fseek(sfp, -width, SEEK_CUR); script_width -= width;
} /* script_erase_input */
...
void record_open(void)
{
	...
	if (new_name != NULL) {
		...
		if ((rfp = modzm_fopen(new_name, "wt")) != NULL)
			ostream_record = TRUE;
		else
			print_string("Cannot open file\n");
	}
...
{
    modzm_fclose (rfp); ostream_record = FALSE;
...
void replay_open(void)
{
	...
	if (new_name != NULL) {
		...
		if ((pfp = modzm_fopen(new_name, "rt")) != NULL) {
