...
static EXT_RAM_BSS_ATTR Zwindow wp[8];
static Zwindow *cwp = wp;
...
{
	if (cwin == 1) return 999; /* Avoid garbling status line. */
...
void z_get_wind_prop(void)
{
	...
	/* Hack for Zork0_r242 */
	if (story_id == ZORK_ZERO && z_header.release == 242 && zargc == 3) {
		if (zargs[2] < 16)
