...
bb_err_t dumb_blorb_init(char *filename)
{
	...
	if ((fp = modzm_fopen(filename, "rb")) == NULL)
		return bb_err_Read;
	...
	if (isblorb(fp)) {			/* Now we know to look */
		...
		if (f_setup.blorb_file != NULL)
			...
			/* Determine default Blorb file name. */
		...
		if ((fp = modzm_fopen(mystring, "rb")) == NULL) {
			p = strrchr(mystring, '.');
			if (p != NULL) *p = '\0';
			strncat(mystring, EXT_BLORB3, len2);
			if (!(fp = modzm_fopen(mystring, "rb")))
		...
		if (f_setup.blorb_file == NULL && !quiet_mode)
			modzm_printf("Found Blorb file named %s.\r\n", mystring);
	...
	*/
	if (f_setup.exec_in_blorb) {
		...
		if (!quiet_mode)
			modzm_printf("Found zcode chunk in Blorb file.\r\n");
	}
...
static int isblorb(FILE *fp)
{
	...
	modzm_fread(mybuf, 1, 4, fp);
	if (strncmp(mybuf, "FORM", 4))
		return 0;

	modzm_fseek(fp, 4, SEEK_CUR);
	modzm_fread(mybuf, 1, 4, fp);
