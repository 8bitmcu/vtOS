...
static char runtime_usage[] =
    ...
    "    \\rn   none    \\rc   CAPS    \\rd   doublestrike    \\ru   "
    "underline\n"
    ...
    "    \\cs      spans: like max, but emit a blank line between each span "
    "of\n"
    "                screen lines shown.\n"
    ...
    "    (Toggle commands can be followed by a 1 or 0 to set value ON or "
    "OFF.)\n"
    "  Character Escapes:\n"
...
static void dumb_getline(char *s) {
  ...
  while (p < s + INPUT_BUFFER_SIZE - 1) {
    c = xgetchar();
    if (c == '\b' || c == 0x7f) {
      /* Backspace: erase the last character if there is one. */
      if (p > s) {
        --p;
  ...
  while ((c = xgetchar()) != '\n')
    ;
  modzm_printf("Line too long, truncated to %s\r\n", s - INPUT_BUFFER_SIZE);
} /* dumb_getline */
...
static bool dumb_read_line(char *s, char *prompt, bool show_cursor, int timeout,
                           enum input_type type, zchar *continued_line_chars) {
  time_t start_time;
    ...
    if (!strcmp(command, "t")) {
      ...
      if (!do_more_prompts)
        ...
        for (;;) {
          ...
          modzm_printf("%.*s", (int)(next_page - current_page), current_page);
          current_page = next_page;
          if (!*current_page)
            break;
          modzm_printf("HELP: Type <return> for more, or q <return> to stop: ");
          fflush(stdout);
          dumb_getline(s);
          if (!strcmp(s, "q\n"))
            break;
...
static EXT_RAM_BSS_ATTR char read_key_buffer[INPUT_BUFFER_SIZE];

/* Similar.  Useful for using function key abbreviations.  */
static EXT_RAM_BSS_ATTR char read_line_buffer[INPUT_BUFFER_SIZE];
...
zchar os_read_line(int UNUSED(max), zchar *buf, int timeout, int UNUSED(width),
                   int continued) {
  char *p;
  int terminator;
  ...
  if (read_line_buffer[0] == '\0') {
    timed_out = dumb_read_line(read_line_buffer, NULL, TRUE, timeout,
                               buf[0] ? INPUT_LINE_CONTINUED : INPUT_LINE, buf);
  } else
    timed_out = check_timeout(timeout);
...
char *os_read_file_name(const char *default_name, int flag) {
  ...
  if (f_setup.restore_mode) {
    ...
    if (f_setup.restricted_path == NULL)
      ...
      buf = modzm_basename((char *)default_name);
  } else {
    if (f_setup.restricted_path != NULL) {
      sprintf(prompt, "Please enter a filename [%s]: ",
              modzm_basename((char *)default_name));
    } else
    ...
    if (strlen(buf) > MAX_FILE_NAME) {
      modzm_printf("Filename too long\r\n");
      free(buf);
      return NULL;
    }
  }
  ...
  if ((flag == FILE_SAVE || flag == FILE_SAVE_AUX || flag == FILE_RECORD) &&
      ((fp = modzm_fopen(file_name, "rb")) != NULL)) {
    modzm_fclose(fp);
...
void os_tick(void) {
   ...
   * active watchdog timers are fed.  modzm_yield(1) calls mp_hal_delay_ms(1)
