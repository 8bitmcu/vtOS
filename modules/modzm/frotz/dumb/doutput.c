...
static EXT_RAM_BSS_ATTR char frotz_to_dumb [256];
...
static void show_cell_irc(cell_t cel)
{
	...
	if (fg != lastfg || bg != lastbg) {
		modzm_putchar ('\017');	/* ^O cancels all text styles */
		lastbold = 0;
		lastemph = 0;

		if (fg != DEFAULT_DUMB_COLOUR) {
			if (bg != DEFAULT_DUMB_COLOUR)
				modzm_printf("\003%u,%s%u", (unsigned char) fg,
					((unsigned char) bg < 10) ? "0" : "", (unsigned char) bg);
			else
				modzm_printf("\003%s%u", ((unsigned char) fg < 10) ? "0" : "", (unsigned char) fg);
		}
	}
	...
	if (cel.style & PICTURE_STYLE)
		modzm_putchar(show_pictures ? cel.c : ' ');
	else
		modzm_putchar(cel.c);
...
static void show_cell_ansi(cell_t cel)
{
	...
	if (fg != lastfg) {
		if (fg < 8)
			modzm_printf("\033[%dm", 30 + fg);
		else
			modzm_printf("\033[38;5;%dm", 232 + fg);
		lastfg = fg;
	}

	if (bg != lastbg) {
		if (bg < 8)
			modzm_printf("\033[%dm", 40 + bg);
		else
			modzm_printf("\033[48;5;%dm", 232 + bg);
		lastbg = bg;
	}
...
static void show_cell_bbcode(cell_t cel)
{
	...
	if (fix != lastfix) {
		modzm_printf("[font=monospace]");
		lastfix = 1;
	}

	if (bg != lastbg) {
		modzm_printf("[bgcolor=%s]", bbcode_colour[(short)bg]);
		lastbg = bg;
	}

	if (fg != lastfg) {
		modzm_printf("[color=%s]", bbcode_colour[(short)fg]);
		lastfg = fg;
	}
...
static void show_cell_normal(cell_t cel)
{
	if (cel.style & REVERSE_STYLE) {
		if (cel.c == ' ')
			modzm_printf("%s", rv_blank_str);
		else {
			switch (rv_mode) {
			case RV_CAPS:
				if (cel.c <= 0x7f) {
					modzm_putchar(toupper(cel.c));
					break;
				}
				/* fall through */
			case RV_NONE:
...
{
	if (visual_bell)
		modzm_printf("[%s-PITCHED BEEP]\n", (volume == 1) ? "HIGH" : "LOW");
	else
		modzm_putchar('\a'); /* so much for dumb.  */
} /* os_beep */
				...
				bool startup)
{
	...
	if (!strncmp(setting, "pb", 2)) {
		...
		modzm_printf("Picture outlines display %s\r\n", show_pictures ? "ON" : "OFF");
		if (startup)
			return TRUE;
		...
		modzm_printf("Compression mode %s, hiding top %d lines\r\n",
		...
		rv_blank_str[1] = 0;
#endif
		modzm_printf("Reverse-video mode %s, blanks reverse to '%s': ",
		...
		modzm_printf("Compression Mode %s, hiding top %d lines\r\n",
			compression_names[compression_mode], hide_lines);
		modzm_printf("Picture Boxes display %s\r\n", show_pictures ? "ON" : "OFF");
		modzm_printf("Visual Bell %s\r\n", visual_bell ? "ON" : "OFF");
		os_beep(1); os_beep(2);
		modzm_printf("Line Numbering %s\r\n", show_line_numbers ? "ON" : "OFF");
		modzm_printf("Line-Type display %s\r\n", show_line_types ? "ON" : "OFF");
