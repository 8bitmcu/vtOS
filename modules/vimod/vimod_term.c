/*
 * vimod_term.c -- replaces neatvi/term.c's terminal I/O with the KVM
...
void term_init(void) {
  ...
  // environment's env.cols/env.rows (see vimod_module.c's constructor),
  // already stashed on current_vimod_instance by the time term_init()
  // runs (vimod_main() sets it up before calling into neatvi's own
  // startup, same ordering as toybox's vi_main()).
  rows = current_vimod_instance ? current_vimod_instance->height : 25;
  cols = current_vimod_instance ? current_vimod_instance->width : 80;
  rows = rows ? rows : 25;
  cols = cols ? cols : 80;
  term_sbuf = sbuf_make();
  term_str("\33[m");
...
static int term_fetch_byte(int timeout_ms) {
  ...
  for (;;) {
    uint8_t byte;
    int errcode;
    vimod_vi_obj_t *vi = current_vimod_instance;
